      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
          <section className="space-y-4">
            <form onSubmit={saveCategory} className="wc-card space-y-3 p-4">
            ...
            <form onSubmit={saveCourse} className="wc-card space-y-3 p-4">
                  ...
                  {categories.map((category) => (
                    <div key={category.id} className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => editCategory(category)} className="flex h-8 w-8 items-center justify-center rounded-lg bg-surface-container p-2 text-primary"><Icon name="edit" /></button>
                        <button onClick={() => deleteRow("course_categories", category.id)} className="flex h-8 w-8 items-center justify-center rounded-lg bg-error-container p-2 text-error"><Icon name="delete" /></button>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[660px] text-left">
                    <thead className="bg-surface-container-low text-[11px] font-bold uppercase tracking-wider text-primary">
                      <tr><th className="px-4 py-3">Course</th><th className="px-4 py-3">Category</th><th className="px-4 py-3">Duration</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Actions</th></tr>
                    </thead>
                    <tbody className="divide-y divide-outline-variant/70">
                      {courses.map((course) => (
                        <motion.tr key={course.id} whileHover={{ backgroundColor: "rgba(0, 33, 110, 0.02)" }}>
                          ...
                          <td className="px-4 py-3"><div className="flex justify-end gap-2"><button onClick={() => editCourse(course)} className="flex h-8 w-8 items-center justify-center rounded-lg bg-surface-container p-2 text-primary"><Icon name="edit" /></button><button onClick={() => deleteRow("courses", course.id)} className="flex h-8 w-8 items-center justify-center rounded-lg bg-error-container p-2 text-error"><Icon name="delete" /></button></div></td>
                        </motion.tr>
                      ))}
                    </tbody>
                  </table>
